...
function Platform({ bg, children, soon }: { bg: string; children: React.ReactNode; soon?: boolean }) {
  return (
    <span className="relative grid h-9 w-9 place-items-center rounded-xl text-white" style={{ background: bg }}>
      {children}
      {soon && (
        <span className="absolute -bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-white px-1.5 text-[8px] font-semibold text-muted shadow-sm">
          Coming Soon
        </span>
      )}
    </span>
...
export default function Hero() {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[#e8f0f7] via-white to-white pt-32 pb-4">
      <div className="mx-auto max-w-4xl px-6 text-center">
        <h1 className="font-display text-5xl text-ink sm:text-6xl md:text-[76px]">
          Creator Marketing That Runs Itself With AI Agents
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-lg text-muted">
          <span className="inline-flex items-center gap-1 font-medium text-ink">🔍 Find, Research &amp; ✈️ Outreach</span>{" "}
          the viral creators that actually fit your app or brand.
        </p>
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Platform bg="linear-gradient(45deg,#f9ce34,#ee2a7b,#6228d7)">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      {/* 動画カルーセル + トグル（フルスクリーン幅・背景なし） */}
      <div className="mt-8 w-full">
        <InsidePovCarousel videos={HERO_VIDEOS} autoRotate interactive />
      </div>

      {/* Get Creators ボタン + Join */}
      <div className="mx-auto mt-6 max-w-4xl px-6 text-center">
        <div className="flex flex-col items-center gap-4">
          <CtaButton size="lg" />
          <div className="flex items-center gap-2 text-sm text-muted">
            <AvatarStack count={3} size={26} start={2} />
